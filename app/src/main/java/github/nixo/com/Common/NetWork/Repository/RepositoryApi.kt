...
interface RepositoryApi{
    ...
    fun allRepositories(@Query("page") page: Int = 1, @Query("q") q: String, @Query("per_page") per_page: Int = 20): rx.Observable<SearchRepositories>
...
//    https://api.github.com/repos/Nixo0427/NixoEditText/contents/?ref=master&uniqueLoginId=Nixo0427   代码预览
    ...
    fun contentsRepositores(@Path("login") login :String, @Path("repo") repos: String, @Path("dir"  ,encoded = true) dir : String ,@Query("ref") branch :String, @Query("uniqueLoginId")uniqueLoginId:String):Observable<List<ContentsRepository>>
